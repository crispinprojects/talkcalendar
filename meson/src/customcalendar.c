...
//void setup_css_classes(CustomCalendar *calendar);
...
//gtk_label_set_label(GTK_LABEL(calendar->day_number_labels[y][x]), prev_month_day_str);
}
...
//gchar *next_month_day_str = "*";
//gtk_label_set_label(GTK_LABEL(calendar->day_number_labels[y][x]), next_month_day_str);
...
{		
//TODO
  //get number days previous month (take year change into account)
// subtract offset
// set calendar day
...
custom_calendar_initialise_eventday_array(calendar);
custom_calendar_initialise_tooltip_array(calendar);
//custom_calendar_goto_dmy(calendar, calendar->day, calendar->month, calendar->year);
