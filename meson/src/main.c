...
g_date_set_time_t(today_date, time(NULL));
int today = g_date_get_day(today_date);
int tmonth = g_date_get_month(today_date);
int tyear = g_date_get_year(today_date);
g_date_free(today_date); // freeit quick

GDate* date =g_date_new_dmy(today, tmonth, tyear);
...
dialog_day_events =gtk_window_new(); 
gtk_window_set_title (GTK_WINDOW (dialog_day_events), date_str);
gtk_window_set_default_size(GTK_WINDOW(dialog_day_events),700,300);
g_object_set_data(G_OBJECT(window), "window-dialog-day-selected-key",dialog_day_events);

header = gtk_header_bar_new ();  
gtk_window_set_titlebar (GTK_WINDOW (dialog_day_events), header);
...
// the listbox callbk_row_activated sets the selected event from the database
// which can then be edited or deleted

GArray *evt_arry_day; //day events	
evt_arry_day = g_array_new(FALSE, FALSE, sizeof(CALENDAR_TYPE_EVENT)); // setup arraylist
db_get_all_events_year_month_day(evt_arry_day, m_start_year,m_start_month, m_start_day);

//loop through day events so that each can be displayed
//using a label appended to listbox allowing selection
for (int i = 0; i < evt_arry_day->len; i++)
{
...
char* end_time_str = get_time_str(end_hour, end_min);
...
g_object_set_data(G_OBJECT(button_delete_event), "dialog-delete-key", dialog_day_events);

gtk_window_set_child (GTK_WINDOW (dialog_day_events),listbox);
gtk_window_present (GTK_WINDOW (dialog_day_events));	
}
//======================================================================
...
GtkWidget *dialog_day_events = g_object_get_data(G_OBJECT(window), "window-dialog-day-selected-key");

gtk_window_destroy(GTK_WINDOW(dialog));
gtk_window_destroy(GTK_WINDOW(dialog_day_events));	
