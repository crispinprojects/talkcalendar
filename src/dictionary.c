	...
	for(int i=0; i < word_number; i++)
	{
	   ...
	   if (g_strcmp0(word_str_lower,"halloween")==0) {
		...
		word_arrays_sizes[i]=movie_raw_len;	
	     ...
	     if (g_strcmp0(word_str_lower,"moving")==0) {		
		word_arrays[i] = (unsigned char*)malloc(moving_raw_len * sizeof(unsigned char));
		word_arrays[i] = moving_raw;
		word_arrays_sizes[i]=moving_raw_len;	
	    }
