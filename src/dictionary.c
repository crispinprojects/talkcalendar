...
void get_words_array(GList *speak_word_list, int word_number,unsigned char **word_arrays, unsigned int *word_arrays_sizes)
{
	...
	for(int i=0; i < word_number; i++)
	{
	   ...
	   if (g_strcmp0(word_str_lower,"nephew")==0) {
	    ...
	    if (g_strcmp0(word_str_lower,"santuary")==0) {
	    word_arrays[i] = (unsigned char*)malloc(santuary_raw_len * sizeof(unsigned char));
		word_arrays[i] =  santuary_raw;		
		word_arrays_sizes[i]=santuary_raw_len; 
	    }
