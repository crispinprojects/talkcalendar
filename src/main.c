...
#define CONFIG_DIRNAME "talkcalendar-rpi"
#define CONFIG_FILENAME "talkcalendar-rpi"
...
static void config_read()
{
	...
	m_talk_event_words=g_key_file_get_integer(kf, "calendar_settings", "speak_event_words", NULL);
...
void config_write()
{
	...
	g_key_file_set_integer(kf, "calendar_settings", "speak_event_words", m_talk_event_words);
...
static void callbk_new_event(GSimpleAction *action, GVariant *parameter,  gpointer user_data)
{
	...
	//mulitday requires end date (to do)
...
static void callbk_update_event(GtkButton *button, gpointer user_data)
{
	...
	//multiday check (to do)
	
	//if ((m_end_year<m_start_year) ||
...
static void callbk_edit_event(GSimpleAction *action, GVariant *parameter,  gpointer user_data)
{
	...
	g_object_set_data(G_OBJECT(button_update), "spin-day-end-key", spin_button_start_day);	
	g_object_set_data(G_OBJECT(button_update), "spin-year-end-key", spin_button_start_year);	
...
static void callbk_about(GSimpleAction * action, GVariant *parameter, gpointer user_data){
	...
	gtk_about_dialog_set_comments(GTK_ABOUT_DIALOG(about_dialog),"Personal Speaking Calendar");
...
static void callbk_set_preferences(GtkButton *button, gpointer  user_data)
{
	...
	m_talk_event_words =gtk_check_button_get_active (GTK_CHECK_BUTTON(check_button_speak_event_words));
...
{	
    ...
    tooltip_str = g_strconcat(tooltip_str, time_str, description_str, "\n",location_str, NULL);
   } //if !all_day	
   else
   {	   
	   //tooltip_str = g_strconcat(tooltip_str, summary_str, "\n",location_str, NULL);
	   tooltip_str = g_strconcat(tooltip_str, description_str, "\n",location_str, NULL);
    }
