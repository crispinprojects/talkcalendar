...
//======================================================================
static void setup_monthname(void)
{
	int i;
	for (i = 0; i < 12; i++)
	{	
...
void custom_calendar_select_day(CustomCalendar *calendar, guint dday, guint month, guint year)
{
	...
	gchar *day_str = g_strdup_printf("%d", calendar->day);
	gchar *year_str = g_strdup_printf("%d", calendar->year);
	gchar *month_str = monthname[calendar->month - 1];
	date_str = g_strconcat(date_str, " ",weekday_str," ",day_str, " ", month_str, " ", year_str, NULL);
	...
	gchar* none_month_day_str="label.none-month-day{background-image: none; font-weight: 100;} ";	
	
	GtkCssProvider *provider_none_month_day; //none month day provider
	provider_none_month_day = gtk_css_provider_new ();
	gtk_css_provider_load_from_string(provider_none_month_day, none_month_day_str); //GTK 4.14
	gtk_style_context_add_provider_for_display (gdk_display_get_default (), GTK_STYLE_PROVIDER (provider_none_month_day), GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
	
	//border: 1px solid #5C6BC0;
	//gchar* frame_str="label.calframe{background-image: none; border-width: 1px;border-style: solid;} ";
	
	gchar* frame_str="label.calframe{background-image: none; border: 1px solid lightgrey;} ";			
	GtkCssProvider *provider_frame;
	provider_frame = gtk_css_provider_new ();
	gtk_css_provider_load_from_string(provider_frame, frame_str); //GTK 4.14
	gtk_style_context_add_provider_for_display (gdk_display_get_default (), GTK_STYLE_PROVIDER (provider_frame), GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
	
	gboolean has_boarder=FALSE;
	
	//gchar* noframe_str="label.noframe{background-image: none; border: none;} ";	
	//GtkCssProvider *provider_noframe; //no frame provider
	//provider_noframe = gtk_css_provider_new ();
	//gtk_css_provider_load_from_string(provider_noframe, noframe_str); //GTK 4.14
	//gtk_style_context_add_provider_for_display (gdk_display_get_default (), GTK_STYLE_PROVIDER (provider_noframe), GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
	 
	//g_print("cc select_day: calendar->has_border =%d\n",calendar->has_border);
	...
	gtk_widget_remove_css_class (GTK_WIDGET(calendar->day_number_labels[y][x]), "none-month-day");
	
	gtk_widget_add_css_class (GTK_WIDGET(calendar->day_number_labels[y][x]), "calframe");
	...
	int days_in_next_month = g_date_get_days_in_month(next_month, next_year);
	...
	if(aday<=0)
	{
	//g_print("previous month aday=%d\n",aday);
	gtk_widget_add_css_class (GTK_WIDGET(calendar->day_number_labels[y][x]), "none-month-day");
	int prev_month_day=days_in_prev_month+aday;
	char* prev_month_day_str=g_strdup_printf("%i", prev_month_day);
	
	char* prev_month_day_str_markup ="<i>";
	prev_month_day_str_markup =g_strconcat(prev_month_day_str_markup, prev_month_day_str,"</i>",NULL);
	gtk_label_set_label(GTK_LABEL(calendar->day_number_labels[y][x]), prev_month_day_str_markup);	
	}
	if(aday >days_in_month)
	{
	//g_print("next month aday=%d\n",aday);
	gtk_widget_add_css_class (GTK_WIDGET(calendar->day_number_labels[y][x]), "none-month-day");
	int next_month_day=aday-days_in_month;
	char* next_month_day_str=g_strdup_printf("%i", next_month_day);
	char* next_month_day_str_markup ="<i>";
	next_month_day_str_markup =g_strconcat(next_month_day_str_markup, next_month_day_str,"</i>",NULL);
	gtk_label_set_label(GTK_LABEL(calendar->day_number_labels[y][x]), next_month_day_str_markup);
	}
...
void custom_calendar_button_press(GtkGestureClick *gesture, int n_press, double x, double y, gpointer user_data)
{
	...
	int days_in_prev_month = g_date_get_days_in_month(calendar->month, calendar->year);	 
	   
	calendar->day =days_in_prev_month+offset_index;
	...
	int day_next_month=offset_index-days_in_month; 
