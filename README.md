...
Synthesized speech can be created by concatenating pieces of recorded speech. The size of the stored speech units can range from  [phonemes](https://en.wikipedia.org/wiki/Phoneme), [diphones](https://en.wikipedia.org/wiki/Diphone), [words](https://en.wikipedia.org/wiki/Speech_synthesis) or even whole sentences. The use of words and sentences is often used in restricted application domains such as public announcement services (e.g. train platform announcements) as it can produce high quality output.

I have developed both word concatenation and diphone concatenation speech synthesizers. The main issue with the word concatenation speech synthesizer approach is that the voice file becomes increasingly larger as more audio word recordings are added to the dictionary.  I had to use Signed 16-bit PCM encoding for the audio files rather than 8000Hz 8-bit Unsigned PCM encoding to eliminate the audio hiss that could heard on playback in previous versions of Talk Calendar. However, this makes the audio word files larger and so adds to the voice file size problem. The voice file containing a few hundred words was over 100MB in size and I received ***WARNING messages*** from GitHub saying that it exceeded and violated their file upload limits. Consequently, I have removed the word concatenation speech synthesizer and stopped using this approach.

Of course, from a file size perspective, the better approach is to use recorded diphones and construct words from these.  With this approach the audio voice file size remains fixed but words have to be constructed by stitching together diphones in the correct order. The quality of a speech synthesizer is judged by its ability to be understood clearly. The word concatenation speech synthesizer approach produced clear fluent speech. The output from my diphone synthesizer speech synthesis lacks some clarity compared to using words as the speech units but the talk rate can be adjusted to help in this respect.
...
A formant speech synthesizer does not construct speech using human speech samples (words, diphones etc.) but uses additive synthesis generating sound by adding together the output of multiple audio generators. I developed a small formant speech synthesizer the details of which can be found [here](https://github.com/crispinprojects/formant-synthesizer). The voice output is very robotic with some acoustic artefacts and so I have not used it for the Talk Calendar project. There is more information on speech synthesis [here](https://en.wikipedia.org/wiki/Speech_synthesis) with references and web links. 

I have also investigated using the [espeak](https://espeak.sourceforge.net/) text-to-speech engine. One issue I had was with word pronunciations. For example, with the word "pilates" the word concatenation synthesizer would you use a pronunciation consisting of the word sounds "pil+laa+teez". Try using espeak to say "pilates". I guess [espeakedit](https://espeak.sourceforge.net/editor.html) could be used to improve the pronunciation. With the diphone synthesizer I can replicate the component sounds to determine how a word is pronounced. 

The speech synthesizer is work in progress.
...
I feel I have been partially successful in my efforts to produce a speaking calendar for Linux with an interface designed for accessibility. Developing a speech synthesizer with human like characteristics has proved more difficult than I originally thought. My word concatenation speech synthesizer did produce clear speech output but was limited by having a small dictionary of words due to file size limitations (see section above on speech synthesis). I received GitHub ***warnings*** that the chipmunk voice file that was used by the word concatenation synthesizer violated and exceeded their file upload limits and so I have removed it and the synthesizer. It has been replaced with my diphone speech synthesizer. More work is need to improve speech clarity with the diphone synthesizer.
